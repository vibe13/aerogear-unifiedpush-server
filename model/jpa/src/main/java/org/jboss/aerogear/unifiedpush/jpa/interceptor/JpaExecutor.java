 ...
 */
package org.jboss.aerogear.unifiedpush.jpa.interceptor;
...
public class JpaExecutor {
    ...
    private final Logger logger = Logger.getLogger(JpaExecutor.class.getName());
    ...
    public <T> T execute(final JpaOperation<T> operation) {
        ...
        try {
            ...
            logger.log(Level.SEVERE, "Error while performing JpaOperation:", cause);

            if (em.getTransaction().isActive()) {
                em.getTransaction().rollback();
            }

            throw new JpaException("Exception while trying to perform JPA operation", cause);
        } finally {
            em.close();
        }
    }
